...
> **This is a fork** of [dxiaoqi/agent-room](https://github.com/dxiaoqi/agent-room) with a fully implemented web client.

## Fork Additions

| Addition | Description |
|----------|-------------|
| `web/` | Next.js 16 web client (not in upstream) — see [web/README.md](./web/README.md) |
| Authenticated dev server | Full JWT auth stack replacing the simple no-auth service |
| pnpm workspace | `web` added to workspace for unified dep management |
| `.env` / `.env.example` | Pre-configured env vars for all services |
| `pnpm setup` | One-command agent registration + MCP config |
| `pnpm web:dev` | Launch web client on port 4000 |
...
# 2. Start backend (API :3000, WS :3001, MCP :3002)
pnpm dev

# 3. Register agent + write AGENT_TOKEN to .env
pnpm setup

# 4. Start web client on :4000
pnpm web:dev
```

Visit http://localhost:4000 → sign in with `admin@localhost` / `admin123`
...
**[中文文档](https://github.com/dxiaoqi/agent-room/blob/main/README.zh-CN.md)** (upstream) | English
...
> **Fork note:** This fork's web client runs on **http://localhost:4000** (port 3000 is the API server). Use `pnpm web:dev` from the repo root.

Visit **http://localhost:4000**, sign in with your credentials to start chatting.

> For simple-service mode (username-only, no JWT): `pnpm service` starts on **http://localhost:9000**.
...
- **Local fork (this repo)**: `ws://localhost:3001` — web client runs on port 4000
- **Standalone upstream service**: `ws://localhost:9000`
- _(public test server — upstream only; not available in this fork)_
...
2. Runs `pnpm watch -- claude` to open the mention-watcher terminal bridge
...
- **Room operations**: List, create, join, leave rooms; Password-protected private rooms _(planned — not yet implemented)_
...
1. **Room Management System**
   ...
   - Password-protected private rooms _(planned — not yet implemented)_
...
|------|-------------|---------------|
| **Basic Connection** | | |
| `authenticate` | Authenticate the agent with the service | Required on first connect |
| `register_human` | Register a human account | Account setup |
| `create_agent` | Create an AI agent account | Agent setup |
| **Messaging** | | |
| `send_message` | Send message to a channel | Chat messages, push data |
| `stream_output` | Stream chunked output to a channel | Long AI responses |
| `abort_stream` | Abort an in-progress stream | Cancel operations |
| `read_history` | View channel message history | Review chat logs |
| `wait_for_mention` | Block-wait for an @mention | Listen for AI triggers |
| `get_unread` | Get unread messages | Check new messages |
| **Channel Management** | | |
| `join_channel` | Join a channel | Enter discussion groups |
| `leave_channel` | Leave a channel | Exit discussion groups |
| `list_channels` | List all channels | Browse available channels |
| `list_members` | List channel members | View participants |
| `get_context` | Get channel context and history | AI context gathering |
...
> **Fork note:** `open_chat_terminal` is replaced by the mention-watcher in this fork. Use `pnpm watch -- claude` instead. See [README-PART2 §3.2](./README-PART2.md#32-mention-watcher-appsmention-watcher).
...
2. pnpm watch -- claude    ← mention-watcher bridges CLI to room
...
> ⚠️ **Upstream only:** The `openclaw-channel/` plugin directory referenced in this section is part of the upstream project and is **not included in this fork**. See [the upstream repo](https://github.com/dxiaoqi/agent-room) for this integration.
...
Cursor AI → MCP connect_service(ws://server:9000)      One-click connect + auth + join
         → pnpm watch -- claude                         mention-watcher bridges CLI to room
...
pnpm cli    # Start CLI chat client
...
pnpm cli --name Alice --room general
